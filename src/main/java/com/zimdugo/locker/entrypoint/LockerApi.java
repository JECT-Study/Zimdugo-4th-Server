...
public interface LockerApi {
    ...
    @Operation(
        summary = "보관함 자동완성 조회",
        description = "검색 점수 우선, 동점 시 현재 좌표 기준 거리순으로 최대 20건의 장소/보관함 자동완성 목록을 반환한다."
    )
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "조회 성공"),
        @ApiResponse(responseCode = "400", description = "잘못된 요청 파라미터")
    ...
    @Operation(
        summary = "보관함 검색 조회",
        description = "검색 점수 우선, 동점 시 현재 좌표 기준 거리순으로 최대 50건의 장소/보관함 검색 결과를 반환한다. "
            + "페이지네이션은 제공하지 않으며, 사이즈, 실내/실외, 보관함 유형 필터를 함께 적용할 수 있다. "
            + "PLACE 결과는 필터에 맞는 거리순 하위 보관함 목록을 반환한다."
    )
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "조회 성공"),
        @ApiResponse(responseCode = "400", description = "잘못된 요청 파라미터")
