...
public record LockerReportCreateRequest(
    ...
    @Schema(description = "ADD_TO_EXISTING 선택 시 필요한 기존 보관함 ID", example = "1")
    Long existingLockerId,

    @Schema(description = "보관함 이름", example = "홍대입구역 2번 출구 보관함")
    @NotBlank
    @Size(max = 100)
    String name,

    @Schema(
        description = "도로명 주소. 주소 변환 실패 시 사용자가 수동으로 입력해야 합니다.",
        example = "서울 마포구 양화로 160"
    )
    @NotBlank
    @Size(max = 255)
    String roadAddress,

    @Schema(description = "상세 위치 설명", example = "2번 출구 안쪽")
    @Size(max = 255)
    ...
    @Schema(description = "실내/실외 유형", example = "INDOOR")
    @Size(max = 20)
    String indoorOutdoorType,

    @Schema(description = "보관함 유형. 값이 없으면 UNKNOWN으로 처리합니다.", example = "UNKNOWN")
    @Size(max = 20)
    String lockerType,
    ...
    @Schema(description = "운영 시간", example = "05:00~24:00")
    @Size(max = 100)
